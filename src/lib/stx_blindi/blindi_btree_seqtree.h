...
#ifndef _STX_BLINDI_BTREE_SEQTREE_H_
#define _STX_BLINDI_BTREE_SEQTREE_H_
...
struct blindi_btree_seqtree_default_set_traits
{
 ...
 * inner node sizes by assuming a cache line size of 256 bytes. */
template <int _InnerSlots, int _LeafSlots>
struct blindi_btree_seqtree_default_map_traits
{
    /// If true, the tree will self verify it's invariants after each insert()
    /// or erase(). The header must have been compiled with BTREE_DEBUG defined.
    static const bool   selfverify = false;
 ...
 */
template <typename _Key, typename _Data, template<typename, int> typename BlindiSeqTreeType,
          int _InnerSlots, int _LeafSlots,
          typename _Value = std::pair<_Key, _Data>,
          typename _Compare = std::less<_Key>,
          typename _Traits = blindi_btree_seqtree_default_set_traits<_InnerSlots, _LeafSlots>,
          bool _Duplicates = false,
          typename _Alloc = std::allocator<_Value>,
          bool _UsedAsSet = false,
          std::uint16_t _Key_len = sizeof(_Key),
          std::uint32_t _Val_len = 8 >
class blindi_btree_seqtree
{
public:
    ...
    typedef GenericBlindiSeqTreeNode<BlindiSeqTreeType<key_type, traits::leafslots>, key_type, traits::leafslots> blindi_node;
...
public:
    ...
    typedef blindi_btree_seqtree<key_type, data_type, BlindiSeqTreeType, _InnerSlots, _LeafSlots, value_type, key_compare,
...
private:
    ...
    struct inner_node : public node
    {
        ...
        /// True if few used enseqtrees, less than half full
        inline bool isfew() const
        {
            return (node::slotuse <= mininnerslots);
        }

        /// True if node has too few enseqtrees
...
public:
    ...
    public:
        ...
        typedef typename blindi_btree_seqtree::key_type                key_type;

        /// The data type of the btree. Returned by data().
        typedef typename blindi_btree_seqtree::data_type               data_type;

        /// The value type of the btree. Returned by operator*().
        typedef typename blindi_btree_seqtree::value_type              value_type;

        /// The pair type of the btree.
        typedef typename blindi_btree_seqtree::pair_type               pair_type;
    ...
    private:
        ...
        typename blindi_btree_seqtree::leaf_node*      currnode;
        ...
        friend class blindi_btree_seqtree<key_type, data_type, BlindiSeqTreeType, _InnerSlots, _LeafSlots, value_type, key_compare,
	...
	/// Default-Constructor of a mutable iterator
        ...
        inline iterator(typename blindi_btree_seqtree::leaf_node *l, unsigned short s)
        ...
        {
            return *((key_type*) currnode->blindi_seqtree.get_key_ptr(currslot));
        }

        /// Writable reference to the current data object -- Artem: it is not writeable???
        inline data_type data() const
        {
            return (data_type) currnode->blindi_seqtree.get_key_ptr(currslot);
            // return *((data_type*) currnode->blindi_seqtree.get_key_ptr(currslot));
        }

        /// Writable reference to the current data  -- added by Artem
        inline data_type& data()
        {
            auto ptr = currnode->blindi_seqtree.get_ptr2key_ptr();
       ...
       /// Prefix++ advance the iterator to the next slot
        ...
        {
            // std::cout << "\nStarting it++:" << std::endl;
            // print_node(std::cout, currnode, 0, true);
            ...
            if (currslot + 1 < currnode->slotuse) {
                ...
                // this is end()
                currslot = currnode->slotuse;
                // std::cout << "reached end(): (" << currnode << ", " << currslot << ")" << std::endl;
                // std::cout << "real end is:   (" << end().currnode << ", " << end().currslot << ")" << std::endl;
            }
            // std::cout << "A print before return:" << std::endl;
            // print_node(std::cout, currnode, 0, true);
            // std::cout << "it++ is done\n" << std::endl;
            return tmp;
        }
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        const typename blindi_btree_seqtree::leaf_node*        currnode;
    ...
    public:
        ...
        inline const_iterator(const typename blindi_btree_seqtree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
    ...
    public:
        ...
        inline reverse_iterator(typename blindi_btree_seqtree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
        ...
        {
            BTREE_ASSERT(currslot > 0);
            return *((key_type*) currnode->blindi_seqtree.get_key_ptr(currslot - 1));
        }
        ...
        {
            BTREE_ASSERT(currslot > 0);
            return (data_type&) currnode->blindi_seqtree.get_key_ptr(currslot - 1);
            // return *((data_type*) currnode->blindi_seqtree.get_key_ptr(currslot - 1));
        }
    ...
    public:
        ...
        inline const_reverse_iterator(const typename blindi_btree_seqtree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
...
public:
    ...
    /// comparison function
    explicit inline blindi_btree_seqtree(const allocator_type &alloc = allocator_type())
    ...
    /// comparison object
    explicit inline blindi_btree_seqtree(const key_compare &kcf,
    ...
    inline blindi_btree_seqtree(InputIterator first, InputIterator last,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc)
    {
        insert(first, last);
    ...
    template <class InputIterator>
    inline blindi_btree_seqtree(InputIterator first, InputIterator last, const key_compare &kcf,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_key_less(kcf), m_allocator(alloc)
    {
...
public:
    ...
    bool exists(const key_type &key) 
    {
        ...
        leaf->blindi_seqtree.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::POINT_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
	 return hit;
    }

    /// SeqTrees to locate a key in the B+ tree and returns an iterator
    ...
    iterator find(const key_type &key)
    {
        ...
        int slot = leaf->blindi_seqtree.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::POINT_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);

        return (hit ? iterator(leaf, slot) : end());
    }

    /// SeqTrees to locate a key in the B+ tree and returns an constant iterator
    /// to the key/data slot if found. If unsuccessful it returns end().
    ...
    /// SeqTrees to locate a key in the B+ tree and returns the number of
    /// identical key enseqtrees found.
// TBD
	...
	bool hit = false, smaller_than_node = false;
        uint16_t tree_traverse[leafslotmax];
        uint16_t tree_traverse_len;
	int slot = leaf->blindi_seqtree.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::PREDECESSOR_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
	...
	int slot = leaf->blindi_seqtree.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::PREDECESSOR_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
        ...
        if(leaf == m_tailleaf && (slot == leaf->blindi_seqtree.node.get_max_pos_node())) {
   	    return end();
	}
...
public:
    ...
    inline blindi_btree_seqtree(const btree_self &other)
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_stats( other.m_stats ),
          m_key_less( other.key_comp() ),
          m_allocator( other.get_allocator() )
...
private:
    ...
    struct node* copy_recursive(const node *n)
    {
        if (n->isleafnode())
        {
            ...
            newleaf.blindi_seqtree = leaf.blindi_seqtree;
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value, bool upsert = false)
    {
	    ...
	    print_node(std::cout, m_root, 0, true);
            verify();
            if(!(exists(key))) {
		std::cout << "key not exists" << std::endl;
		exit(0);
	    }	
        }
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode, bool upsert = false)
    {
        ...
        else // n->isleafnode() == true
        {
            ...
            if (b_split && leaf != *splitnode && key_greater(key, leaf->blindi_seqtree.get_key_ptr(leafslotmax/2 - 1)))
            {
            ...
            auto ins_res = (upsert)
                ? leaf->blindi_seqtree.Upsert2BlindiNodeWithKey((uint8_t*) &key, _Key_len)
                : leaf->blindi_seqtree.Insert2BlindiNodeWithKey((uint8_t*) &key, _Key_len);
            if (ins_res == INSERT_DUPLICATED_KEY){
                auto result = std::pair<iterator, bool>(iterator(leaf, slot), false);
                // fprintf(stderr, "----> Returning from insert with slot = %u\n", slot);
                return result;
                // return std::pair<iterator, bool>(iterator(leaf, slot), false);
            }
            leaf->slotuse++;

            auto result = std::pair<iterator, bool>(iterator(leaf, slot), true);
            // fprintf(stderr, "----> Returning from insert with slot = %u\n", slot);
            return result;
...
//        *_newkey = leaf->blindi_seqtree.get_mid_key_in_node();

        blindi_node small_tree;

        leaf->blindi_seqtree.SplitBlindiNode(&small_tree, &newleaf->blindi_seqtree);

        memcpy(&leaf->blindi_seqtree, &small_tree, sizeof(leaf->blindi_seqtree));

        leaf->slotuse = mid;
        leaf->nextleaf = newleaf;
        newleaf->prevleaf = leaf;

        *_newkey = leaf->blindi_seqtree.get_max_key_in_node();
        *_newleaf = newleaf;
    }
...
private:
                               ...
                               inner_node *parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            leaf->slotuse--;
            leaf->blindi_seqtree.RemoveFromBlindiNodeWithKey((uint8_t*) &key, _Key_len);
            ...
            if (slot == leaf->slotuse)
            {
			    ...
			    parent->slotkey[parentslot] = *((_Key*)leaf->blindi_seqtree.get_key_ptr(leaf->slotuse - 1));
		    }	
                }
                else
                {
                    if (leaf->slotuse >= 1)
                    {
                        BTREE_PRINT("Scheduling lastkeyupdate: key " << *((_Key*)leaf->blindi_seqtree.get_key_ptr(leaf->slotuse - 1)));
                        myres |= result_t(btree_update_lastkey, *((_Key*)leaf->blindi_seqtree.get_key_ptr(leaf->slotuse - 1)));
                    }
                    else
                    {
	    ...
	    if (result.has(btree_fixmerge))
            {
                ...
                if (inner->level == 1)
                {
                    ...
                    inner->slotkey[slot] = child->blindi_seqtree.get_max_key_in_node();
                }
            }
...
//        std::cout << " Merge leaf nodes " << left->slotuse << " " << (int)left->blindi_seqtree.node.get_valid_len() << " and " << (int)right->blindi_seqtree.node.get_valid_len() << " with common parent " << parent << std::endl;
        (void)parent;
...
*/
	left->blindi_seqtree.MergeBlindiNodes(&right->blindi_seqtree);
    ...
    static result_t merge_inner(inner_node* left, inner_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        // reseqtreeve the decision key from parent
...
/*
        ...
        BTREE_PRINT("Shifting (leaf) " << shiftnum << " enseqtrees to left " << left << " from right " << right << " with common parent " << parent << ".");
...
*/
        ...
        if (parentslot < parent->slotuse) {
            parent->slotkey[parentslot] = left->blindi_seqtree.get_max_key_in_node();
            return btree_ok;
        }
        else { // the update is further up the tree
            return result_t(btree_update_lastkey, left->blindi_seqtree.get_max_key_in_node());
        }
    }
    ...
    static void shift_left_inner(inner_node *left, inner_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        BTREE_PRINT("Shifting (inner) " << shiftnum << " enseqtrees to left " << left << " from right " << right << " with common parent " << parent << ".");
...
/*        BTREE_ASSERT(left->isleafnode() && right->isleafnode());
        ...
        BTREE_PRINT("Shifting (leaf) " << shiftnum << " enseqtrees to right " << right << " from left " << left << " with common parent " << parent << ".");
...
//	    }
		...
		key_type _large_key = key_type();
                _small_key = *((_Key*)leaf->blindi_seqtree.get_key_ptr(slot));
		_large_key =  *((_Key*)leaf->blindi_seqtree.get_key_ptr(slot+1));
                if (key_greater(_small_key, _large_key)) {
			std::cout << "key_greater(_small_key, _large_key) " << std::endl;
			exit(0);
		}
            }
	    *minkey = leaf->blindi_seqtree.get_min_key_in_node();
	    *maxkey = leaf->blindi_seqtree.get_max_key_in_node();		
	    
	    if (leaf->nextleaf != NULL) { 	 
		    key_type _max_key = key_type();
		    key_type _next_key = key_type();
		    _max_key = leaf->blindi_seqtree.get_max_key_in_node();
		    _next_key =  leaf->nextleaf->blindi_seqtree.get_min_key_in_node();
    ...
    {
	std::cout << "verify_leaflinks " << std::endl;
        const leaf_node *n = m_headleaf;
        ...
        while(n)
        {
            ...
            {
                assert(key_lessequal(n->blindi_seqtree.get_key_ptr(slot), n->blindi_seqtree.get_key_ptr(slot + 1)));
            }
            ...
            if (n->nextleaf)
            {
                assert(key_lessequal(n->blindi_seqtree.get_key_ptr(n->slotuse - 1), n->nextleaf->blindi_seqtree.get_key_ptr(0))); 
...
#endif // _STX_BLINDI_BTREE_SEQTREE_H_
