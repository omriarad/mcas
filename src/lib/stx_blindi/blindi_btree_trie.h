...
#ifndef _STX_BLINDI_BTREE_TRIE_H_
#define _STX_BLINDI_BTREE_TRIE_H_
...
struct blindi_btree_trie_default_set_traits
{
 ...
 * inner node sizes by assuming a cache line size of 256 bytes. */
template <int _InnerSlots, int _LeafSlots>
struct blindi_btree_trie_default_map_traits
{
    /// If true, the tree will self verify it's invariants after each insert()
    /// or erase(). The header must have been compiled with BTREE_DEBUG defined.
    static const bool   selfverify = false;
 ...
 */
template <typename _Key, typename _Data, template<typename, int> typename BlindiTrieType,
          int _InnerSlots, int _LeafSlots,
          typename _Value = std::pair<_Key, _Data>,
          typename _Compare = std::less<_Key>,
          typename _Traits = blindi_btree_trie_default_set_traits<_InnerSlots, _LeafSlots>,
          bool _Duplicates = false,
          typename _Alloc = std::allocator<_Value>,
          bool _UsedAsSet = false,
          std::uint16_t _Key_len = sizeof(_Key),
          std::uint32_t _Val_len = 8 >
class blindi_btree_trie
{
public:
    ...
    typedef GenericBlindiTrieNode<BlindiTrieType<key_type, traits::leafslots>, key_type, traits::leafslots> blindi_node;
...
public:
    ...
    typedef blindi_btree_trie<key_type, data_type, BlindiTrieType, _InnerSlots, _LeafSlots, value_type, key_compare,
...
public:
    ...
    public:
        ...
        typedef typename blindi_btree_trie::key_type                key_type;

        /// The data type of the btree. Returned by data().
        typedef typename blindi_btree_trie::data_type               data_type;

        /// The value type of the btree. Returned by operator*().
        typedef typename blindi_btree_trie::value_type              value_type;

        /// The pair type of the btree.
        typedef typename blindi_btree_trie::pair_type               pair_type;
        ...
        uint16_t tree_traverse[leafslotmax];
    ...
    private:
        ...
        typename blindi_btree_trie::leaf_node*      currnode;
        ...
        friend class blindi_btree_trie<key_type, data_type, BlindiTrieType, _InnerSlots, _LeafSlots, value_type, key_compare,
                                 traits, allow_duplicates, allocator_type, used_as_set, _Key_len, _Val_len>;
    ...
    public:
        ...
        inline iterator(typename blindi_btree_trie::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }

        inline iterator(typename blindi_btree_trie::leaf_node *l, unsigned short s, uint16_t tt_l, uint16_t *tt)
            : currnode(l), currslot(s), tree_traverse_len (tt_l)
        { for (int i=0; i < tt_l; i++) {
		tree_traverse[i] = tt[i];
	  }	
	}


        /// Initializing-Constructor of a mutable iterator
        inline iterator(typename blindi_btree_trie::leaf_node *l)
                : currnode(l) {
            currslot = findMinSortedSlot();
        }
        ...
        {
            return *((key_type*) currnode->blindi_trie.get_key_ptr(currslot));
        }

        /// Writable reference to the current data object
        inline data_type data() const
        {
            return *((data_type*) currnode->blindi_trie.get_key_ptr(currslot));
        }

        inline unsigned short findNextSortedSlot() {
            auto max_pos = currnode->blindi_trie.node.get_max_pos_node();
	    if (this->currslot != max_pos) {
            	return currnode->blindi_trie.get_next_item(this->tree_traverse, &this->tree_traverse_len);
	    }
            return leafslotmax;
        }
	
 	inline unsigned short findPreviousSortedSlot() {
            auto min_pos = currnode->blindi_trie.node.get_min_pos_node();
	    if (this->currslot != min_pos) {
            	return currnode->blindi_trie.get_previous_item(this->tree_traverse, &this->tree_traverse_len);
	    }
            return leafslotmax;
        }
        inline unsigned short findMinSortedSlot() {
            if (currnode == NULL) return 0;
            if (currnode->blindi_trie.get_valid_len() == 0) return leafslotmax;
            return currnode->blindi_trie.node.get_min_pos_node(this->tree_traverse, &this->tree_traverse_len);
        }

/*
        inline unsigned short findMaxSortedSlotReversed() {
            uint8_t sorted_items[leafslotmax];
            for (int i = 0; i < leafslotmax; i++) {
                sorted_items[i] = leafslotmax;
            }
            currnode->blindi_trie.bring_sorted_items(sorted_items);
            for (int i = 1; i < leafslotmax; i++) {
                if (sorted_items[i] == currslot) {
...
*/
        ...
        {
            int next_sorted_slot = findNextSortedSlot();
            if (next_sorted_slot < leafslotmax) {
                currslot = next_sorted_slot;
            }
        ...
        {
            int pre_sorted_slot = findPreviousSortedSlot();
            if (pre_sorted_slot < leafslotmax) {
                currslot = pre_sorted_slot;
            }
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        const typename blindi_btree_trie::leaf_node*        currnode;
    ...
    public:
        ...
        inline const_iterator(const typename blindi_btree_trie::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
        ...
        inline self& operator++()
        {
            ...
            std::cout << "ERROR: not implemantaed function:  inline self& operator++() " << std::endl;
            return *this;
        }
    ...
    public:
        ...
        inline reverse_iterator(typename blindi_btree_trie::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
        ...
        {
            BTREE_ASSERT(currslot > 0);
            return *((key_type*) currnode->blindi_trie.get_key_ptr(currslot - 1));
        }
        ...
        {
            BTREE_ASSERT(currslot > 0);
            return *((data_type*) currnode->blindi_trie.get_key_ptr(currslot - 1));
        }
    ...
    public:
        ...
        inline const_reverse_iterator(const typename blindi_btree_trie::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
...
public:
    ...
    struct tree_stats
    {
        ...
        inline tree_stats()
            : itemcount(0),
              leaves(0), innernodes(0), indexcapacity(0)
        {
        }
...
public:
    ...
    /// comparison function
    explicit inline blindi_btree_trie(const allocator_type &alloc = allocator_type())
    ...
    /// comparison object
    explicit inline blindi_btree_trie(const key_compare &kcf,
    ...
    inline blindi_btree_trie(InputIterator first, InputIterator last,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc)
    {
        insert(first, last);
    ...
    template <class InputIterator>
    inline blindi_btree_trie(InputIterator first, InputIterator last, const key_compare &kcf,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_key_less(kcf), m_allocator(alloc)
    {
...
public:
    ...
    inline iterator end()
    {
        return iterator(m_tailleaf, m_tailleaf ? leafslotmax : 0);
    }
    ...
    {
        return const_iterator(m_headleaf);
    }
    ...
    {
        return const_iterator(m_tailleaf, m_tailleaf ? leafslotmax : 0);
    }
...
private:
    ...
    inline int find_lower(const inner_node *n, const key_type& key) const
    {
...
public:
    ...
    bool exists(const key_type &key) 
    {
        ...
        leaf->blindi_trie.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::POINT_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
        return hit;
    }
    ...
    iterator find(const key_type &key)
    {
        ...
        int slot = leaf->blindi_trie.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::POINT_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
    ...
    iterator lower_bound(const key_type& key)
    {
        ...
        uint16_t tree_traverse_len;
	bool hit = false, smaller_than_node = false;
	int slot = leaf->blindi_trie.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::PREDECESSOR_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
        if (hit || smaller_than_node)  {
		return iterator(leaf, slot, tree_traverse_len, tree_traverse);
	}
        if(leaf == m_tailleaf && (slot == leaf->blindi_trie.node.get_max_pos_node())) {
   	    return end();
	}
	slot = leaf->blindi_trie.get_next_item(tree_traverse, &tree_traverse_len);    
	return iterator(leaf, slot, tree_traverse_len, tree_traverse);
    }
	...
	bool hit = false, smaller_than_node = false;
        uint16_t tree_traverse[leafslotmax];
        uint16_t tree_traverse_len;
	uint16_t slot = leaf->blindi_trie.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::PREDECESSOR_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
        if (hit || smaller_than_node)  {
		return iterator(leaf, slot, tree_traverse_len, tree_traverse);
	}
        if(leaf == m_tailleaf && (slot == leaf->blindi_trie.node.get_max_pos_node())) {
	...
	iterator tt (leaf, slot, tree_traverse_len, tree_traverse);
	tt++;
	return tt;
    }
...
public:
    ...
    inline blindi_btree_trie(const btree_self &other)
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_stats( other.m_stats ),
          m_key_less( other.key_comp() ),
          m_allocator( other.get_allocator() )
...
private:
    ...
    struct node* copy_recursive(const node *n)
    {
        if (n->isleafnode())
        {
            ...
            newleaf.blindi_trie = leaf.blindi_trie;
...
public:
    ...
    inline std::pair<iterator, bool> upsert(const key_type& key, const data_type& data)
    {
        return insert_start(key, data, true);
    }
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value, bool upsert = false)
    {
        node *newchild = NULL;
        ...
        std::pair<iterator, bool> r = insert_descend(m_root, key, value, &newkey, &newchild, upsert);
	    ...
	    print_key (std::cout, (uint8_t*) &key);	
	    std::cout << ")" << std::endl;  
	    std::cout << " print node " << std::endl;
	    print_node(std::cout, m_root, 0, true);
            verify();
            if(!(exists(key))) {
		std::cout << "key not exists after insert" << std::endl;
		exit(0);
	    }	
        }
        return r;
    ...
    */
    std::pair<iterator, bool> insert_descend(node* n,
                                             ...
                                             key_type* splitkey, node** splitnode, bool upsert = false)
    {
        if (!n->isleafnode())
        {
            ...
            std::pair<iterator, bool> r = insert_descend(inner->childid[slot],
                                                         key, value, &newkey, &newchild, upsert);
        ...
        else // n->isleafnode() == true
        {
            ...
            if (b_split && leaf != *splitnode && key_greater(key, leaf->blindi_trie.get_key_ptr(leafslotmax/2 - 1))) //TODO: change to get_mid_in_node()
            {
            ...
            int ins_res = (upsert)
                ? leaf->blindi_trie.Upsert2BlindiNodeWithKey((uint8_t*) &key, _Key_len)
                : leaf->blindi_trie.Insert2BlindiNodeWithKey((uint8_t*) &key, _Key_len);
            if (ins_res == INSERT_DUPLICATED_KEY){
                return std::pair<iterator, bool>(iterator(leaf, slot), false);
            }
            leaf->slotuse++;
    ...
    void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        ...
        *_newkey = leaf->blindi_trie.get_mid_key_in_node();

        blindi_node small_tree;

        leaf->blindi_trie.SplitBlindiNode(&small_tree, &newleaf->blindi_trie);

        memcpy(&leaf->blindi_trie, &small_tree, sizeof(leaf->blindi_trie));

        leaf->slotuse = mid;
        leaf->nextleaf = newleaf;
        newleaf->prevleaf = leaf;

        *_newkey = leaf->blindi_trie.get_max_key_in_node();
        *_newleaf = newleaf;
    }
...
public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        for (size_t i = 0; i < num_leaves; ++i)
        {
            ...
            for (size_t s = 0; s < leaf->slotuse; ++s, ++it); // DEBUG HEZI
                //leaf->set_slot(s, *it);
...
public:
    ...
    bool erase_one(const key_type &key)
    {
        ...
        if (!m_root || (m_root->slotuse == 0)) return false; // add a condition to support m_root->slotuse == 0
	...
	if (selfverify) {
		std::cout << "erase the key";
		print_key (std::cout, (uint8_t*) &key);	
	    std::cout << " print node " << std::endl;
	    print_node(std::cout, m_root, 0, true);
            verify();
            if((exists(key))) {
		std::cout << "key exists after remove" << std::endl;
		exit(0);
	    }	
        }   
    ...
    void erase(iterator iter)
    {
        ...
        result_t result = btree_not_found;//erase_iter_descend(iter, m_root, NULL, NULL, NULL, NULL, NULL, 0);
...
private:
                               ...
                               inner_node *parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            uint16_t tree_traverse[leafslotmax];
            uint16_t tree_traverse_len;
            uint16_t slot = leaf->blindi_trie.SearchBlindiNode((uint8_t*) &key, _Key_len, SEARCH_TYPE::POINT_SEARCH, &hit, &smaller_than_node, tree_traverse, &tree_traverse_len);
            ...
            leaf->slotuse--;
            int _max_key_slot_bf_erase = leaf->blindi_trie.node.get_max_pos_node();
            leaf->blindi_trie.RemoveFromBlindiNodeWithKey((uint8_t*) &key, _Key_len);
            result_t myres = btree_ok;
            // if the last key of the leaf was changed, the parent is notified
            // and updates the key of this leaf
            if (slot == _max_key_slot_bf_erase)
            {
                if (parent && parentslot < parent->slotuse)
                {
                    BTREE_ASSERT(parent->childid[parentslot] == curr);
                    if (leaf->slotuse > 0) {  // if slotuse == 0, the leaf is empty 
			    parent->slotkey[parentslot] = leaf->blindi_trie.get_max_key_in_node();
		    }	
                }
                else
                {
                    if (leaf->slotuse >= 1)
                    {
                        myres |= result_t(btree_update_lastkey, leaf->blindi_trie.get_max_key_in_node());
                    }
                    else
                    {
                        BTREE_ASSERT(leaf == m_root);
		        std::cout << " The tree is empty " <<  " slot " << slot << " slotuse  " << leaf->slotuse <<  std::endl;  
		        assert (leaf == m_root);
                    }
                }
            }
	    ...
	    if (leaf->isunderflow())
	    {
		    ...
		    if (leaf == m_root && leaf->slotuse == 0 && leftleaf == NULL && rightleaf == NULL)
		    {
			    BTREE_ASSERT(leaf == m_root);
		    ...
		    if (leaf != m_root)
		    {
			    // case : merge if left_leaf + leaf < soltuse and they have the same parent  
			    if ((leftleaf != NULL) && ((leaf->slotuse + leftleaf->slotuse) <= leafslotmax) && ( leftparent == parent)) { 
				    myres |= merge_leaves(leftleaf, leaf, leftparent);
			    }
			    // case : merge if right_leaf + leaf < soltuse and they have the same parent  
			    else if ((rightleaf != NULL) && ((leaf->slotuse + rightleaf->slotuse) <= leafslotmax) && ( rightparent == parent)) { 
			    ...
			    else if (leaf->slotuse == 0 && leftparent == rightparent)
			    {
			    ...
			    {
			    if (leaf->slotuse == 0 && leftparent == parent)
			    shift_right_leaf(leftleaf, leaf, leftparent, parentslot - 1);
			    else if (leaf->slotuse == 0 && leftparent != parent)
			    myres |= shift_left_leaf(leaf, rightleaf, rightparent, parentslot);
			    }
			     */
		    }
	    ...
	    if (result.has(btree_fixmerge))
            {
                ...
                if (inner->level == 1)
                {
                    ...
                    inner->slotkey[slot] = child->blindi_trie.get_max_key_in_node();
                }
            }
...
//        std::cout << " Merge leaf nodes " << left->slotuse << " " << (int)left->blindi_trie.node.get_valid_len() << " and " << (int)right->blindi_trie.node.get_valid_len() << " with common parent " << parent << std::endl;
        (void)parent;
...
*/
	left->blindi_trie.MergeBlindiNodes(&right->blindi_trie);
...
/*
        ...
        BTREE_PRINT("Shifting (leaf) " << shiftnum << " entrees to left " << left << " from right " << right << " with common parent " << parent << ".");
...
*/
        ...
        if (parentslot < parent->slotuse) {
            parent->slotkey[parentslot] = left->blindi_trie.get_max_key_in_node();
            return btree_ok;
        }
        else { // the update is further up the tree
            return result_t(btree_update_lastkey, left->blindi_trie.get_max_key_in_node());
        }
    }
    ...
    static void shift_left_inner(inner_node *left, inner_node *right, inner_node *parent, unsigned int parentslot)
    {
        ...
        BTREE_PRINT("Shifting (inner) " << shiftnum << " entrees to left " << left << " from right " << right << " with common parent " << parent << ".");
...
/*        BTREE_ASSERT(left->isleafnode() && right->isleafnode());
        ...
        BTREE_PRINT("Shifting (leaf) " << shiftnum << " entrees to right " << right << " from left " << left << " with common parent " << parent << ".");
...
private:
    ...
    static void print_node(std::ostream &os, const node* node, unsigned int depth=0, bool recursive=false)
    {
        ...
        if (node->isleafnode())
        {
            ...
            for (unsigned int slot = 0; slot < leafnode->slotuse; ++slot)
            {
		print_key (os, leafnode->blindi_trie.node.get_key_ptr(slot));
            }
            os << std::endl;

	    uint16_t sorted_positions[leafslotmax];
	    leafnode->blindi_trie.node.bring_sorted_items(sorted_positions);
            os << "  sorted positions ";
	    for (unsigned int slot = 0; slot < leafnode->slotuse; ++slot)
            {
            	os << "  (" << (int)sorted_positions[slot] << ")";
            }
	    os << std::endl;	
...
public:
    ...
    void verify() const
    {
        ...
        if (m_root)
        {
            ...
            if (!(vstats.itemcount == m_stats.itemcount &&   vstats.leaves == m_stats.leaves &&  vstats.innernodes == m_stats.innernodes)) {
                std::cout << "vstats ( " << vstats.itemcount << "," << vstats.leaves << "," << vstats.innernodes << ") != m_stats (" << m_stats.itemcount << "," << m_stats.leaves << "," << m_stats.innernodes << ")" << std::endl;
		exit(0);
	    } 
            verify_leaflinks();
        }
...
private:
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey, tree_stats &vstats) const
    {
        ...
        std::cout << " verifynode " << n << std::endl;
...
//	    }
	    ...
	    uint16_t sorted_positions[leafslotmax];
	    leaf->blindi_trie.node.bring_sorted_items(sorted_positions);
	
            for(unsigned short slot = 0; slot < leaf->slotuse - 1; ++slot)
            {
		key_type _small_key = key_type();
		key_type _large_key = key_type();
                _small_key = *((_Key*)leaf->blindi_trie.get_key_ptr(sorted_positions[slot]));
		_large_key =  *((_Key*)leaf->blindi_trie.get_key_ptr(sorted_positions[slot+1]));
                if (key_greater(_small_key, _large_key)) {
			std::cout << "key_greater(_small_key, _large_key) " << std::endl;
			exit(0);
		}
            }
	    *minkey = leaf->blindi_trie.get_min_key_in_node();
	    *maxkey = leaf->blindi_trie.get_max_key_in_node();		
	    
	    if (leaf->nextleaf != NULL) { 	 
		    key_type _max_key = key_type();
		    key_type _next_key = key_type();
		    _max_key = leaf->blindi_trie.get_max_key_in_node();
		    _next_key =  leaf->nextleaf->blindi_trie.get_min_key_in_node();
	...
	else // !n->isleafnode()
	{
		...
		for(unsigned short slot = 0; slot < inner->slotuse - 1; ++slot)
		{
			...
			{
				std::cout << " key_greater(inner->slotkey[slot+1], inner->slotkey[slot] " << std::endl;
				exit(0);
			}
		}
			...
			if (slot == 0)
			{
				*minkey = subminkey;
				...
				if(!(key_greaterequal(subminkey, inner->slotkey[slot-1]))) {
					std::cout << " !(key_greaterequal(subminkey, inner->slotkey[slot-1])) " << std::endl;
					exit(0);
				}
    ...
    void verify_leaflinks() const
    {
        ...
        while(n)
        {
            ...
            {
                assert(key_lessequal(n->blindi_trie.get_key_ptr(slot), n->blindi_trie.get_key_ptr(slot + 1)));
            }
            ...
            if (n->nextleaf)
            {
                assert(key_lessequal(n->blindi_trie.get_key_ptr(n->slotuse - 1), n->nextleaf->blindi_trie.get_key_ptr(0))); //DEBUG HEZI
...
#endif // _STX_BLINDI_BTREE_TRIE_H_
