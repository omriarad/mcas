 ...
 */
template <typename _Key, typename _Data, int _InnerSlots, int _LeafSlots,
          typename _Value = std::pair<_Key, _Data>,
          typename _Compare = std::less<_Key>,
          typename _Traits = btree_default_map_traits<_InnerSlots, _LeafSlots>,
...
public:
    ...
    typedef btree<key_type, data_type, _InnerSlots, _LeafSlots, value_type, key_compare,
...
public:
    ...
    private:
        ...
        friend class btree<key_type, data_type, _InnerSlots, _LeafSlots, value_type, key_compare,
...
private:
    ...
    inline leaf_node* allocate_leaf(int slotsize)
    {
...
public:
    ...
    {
#ifdef BREATHING_SIZE 
//	if(!(m_stats.itemcount % (1024*1024)))
//	{
		std::cout << " avgslotsize "<< m_stats.avgfill_leavesslotbreath() << " breathing_num "<< m_stats.breathing_num <<  " "; 
//	}
#endif	
        return m_stats;
    }
...
private:
    ...
    struct node* copy_recursive(const node *n)
    {
        ...
        {
            const leaf_node *leaf = static_cast<const leaf_node*>(n);
            leaf_node *newleaf = allocate_leaf(leafslotmax);
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value)
    {
        ...
        if (m_root == NULL) {
            m_root = m_headleaf = m_tailleaf = allocate_leaf(leafslotmax);
#ifdef BREATHING_SIZE 
    ...
    */
	...
	leaf_node *newleaf;
		...
		m_stats.leavesslotsbreath += leafslotmax;
        	newleaf = allocate_leaf(leafslotmax);
       }
	else 
       {
		m_stats.leavesslotsbreath += (minleafslots + BREATHING_SIZE);
        	newleaf = allocate_leaf(minleafslots + BREATHING_SIZE);
	}	
#else
        leaf_node *newleaf = allocate_leaf(leafslotmax);
#endif
...
public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        for (size_t i = 0; i < num_leaves; ++i)
        {
            ...
            leaf_node* leaf = allocate_leaf(leafslotmax);
