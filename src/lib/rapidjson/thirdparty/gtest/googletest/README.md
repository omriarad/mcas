...
New file `CMakeLists.txt.in`:
    ...
    ExternalProject_Add(googletest
      ...
      GIT_TAG           master
      SOURCE_DIR        "$ENV{MCAS_BINARY_DIR}/googletest-src"
      BINARY_DIR        "$ENV{MCAS_BINARY_DIR}/googletest-build"
      CONFIGURE_COMMAND ""
      BUILD_COMMAND     ""
      INSTALL_COMMAND   ""
      TEST_COMMAND      ""
...
Existing build's `CMakeLists.txt`:
    ...
    execute_process(COMMAND ${CMAKE_COMMAND} -G "${CMAKE_GENERATOR}" .
      ...
      WORKING_DIRECTORY $ENV{MCAS_BINARY_DIR}/googletest-download )
    if(result)
    ...
    add_subdirectory($ENV{MCAS_BINARY_DIR}/googletest-src
                     $ENV{MCAS_BINARY_DIR}/googletest-build
                     EXCLUDE_FROM_ALL)
