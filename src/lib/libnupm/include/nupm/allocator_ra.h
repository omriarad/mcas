...
namespace nupm
{
	...
	template <typename T, typename MR>
		...
		public:
			...
			auto allocate(
				size_type s
				, typename allocator_adaptor<void, MR>::const_pointer /* hint */ =
					typename allocator_adaptor<void, MR>::const_pointer{}
				, const char * = nullptr
			) -> pointer
			{
				auto ptr = this->pmr()->alloc(s * sizeof(T), alignof(T))->addr();
				if ( ptr == 0 )
				{
