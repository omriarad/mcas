...
) -> region_descriptor
{
  ...
  {
    CPLOG(0,"%s: path %s id %.*s size req 0x%zx create failed (available 0x%zx)", __func__, arena->describe().data(), int(name_.size()), name_.begin(), size_, arena->get_max_available());
