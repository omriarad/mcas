...
class ExperimentPut : public Experiment {
 ...
 public:
  ExperimentPut(const ProgramOptions &options)
      : Experiment("put", options), _i(0), _start_time(), _latencies(), _latency_stats(test_name())
  {
  }
  ...
  void cleanup_custom(unsigned core) override
  {
    ...
    try {
      ...
      if (is_verbose()) {
        ...
        stats_info << "creating time_stats with " << bin_count() << " bins: [" << _start_time.front() << " - "
                   << _start_time.at(_i - 1) << "]. _i = " << _i << std::endl;
      ...
      PINF("[%u] put: THROUGHPUT: %.2f MB/s (%lu bytes over %.3f seconds)", core, throughput, total_data_processed(),
           run_time);

      if (is_json_reporting()) {
        // compute _start_time_stats pre-lock
        BinStatistics start_time_stats = _compute_bin_statistics_from_vectors(
            _latencies, _start_time, bin_count(), _start_time.front(), _start_time.at(_i - 1), _i);
        ...
        experiment_object.AddMember("IOPS", double(iops), document.GetAllocator())
