...
namespace eastl
{
	...
	struct ListNodeBase
	{
		tracked<ListNodeBase*, 'L'> mpNext;
		tracked<ListNodeBase*, 'L'> mpPrev;
	...
	inline ListIterator<T, Pointer, Reference>::ListIterator(const ListNodeBase* pNode) EA_NOEXCEPT
		: mpNode(
			static_cast<node_type*>(
				static_cast<ListNode<T>*>(const_cast<ListNodeBase*>(pNode))
			)
	...
	{
#if 0
		mpNode = tracked_static_ptr_cast<node_type>(mpNode->mpNext);
#else
