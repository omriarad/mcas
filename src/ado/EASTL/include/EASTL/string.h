...
//
	...
	namespace eastl
	{
		inline int Vsnprintf(char8_t* pDestination, size_t n, const char8_t* pFormat, va_list arguments)
			{
				auto ct = Vsnprintf8(pDestination, n, pFormat, arguments);
				if ( 0 < ct ) { tracking::track_raw(pDestination, std::min(std::size_t(ct+1), n) * sizeof *pDestination); }
				return ct;
			}

		inline int Vsnprintf(char16_t* pDestination, size_t n, const char16_t* pFormat, va_list arguments)
			{
				auto ct = Vsnprintf16(pDestination, n, pFormat, arguments);
				if ( 0 < ct ) { tracking::track_raw(pDestination, std::min(std::size_t(ct+1), n) * sizeof *pDestination); }
				return ct;
			}
		...
		inline int Vsnprintf(char32_t* pDestination, size_t n, const char32_t* pFormat, va_list arguments)
			{
				auto ct = Vsnprintf32(pDestination, n, pFormat, arguments);
				if ( 0 < ct ) { tracking::track_raw(pDestination, std::min(std::size_t(ct+1), n) * sizeof *pDestination); }
				return ct;
			}
			...
			inline int Vsnprintf(wchar_t* pDestination, size_t n, const wchar_t* pFormat, va_list arguments)
				{
					auto ct = VsnprintfW(pDestination, n, pFormat, arguments);
					if ( 0 < ct ) { tracking::track_raw(pDestination, std::min(std::size_t(ct+1), n) * sizeof *pDestination); }
					return ct;
				}
		#endif
...
namespace eastl
{
	...
	protected:
		...
		struct HeapLayout
		{
			tracked<value_type*, 'b'> mpBegin;  // Begin of string.
			tracked<size_type, 's'> mnSize;     // Size of the string. Number of characters currently in the string, not including the trailing '0'
			tracked<size_type, 'k'> mnCapacity; // Capacity of the string. Number of characters string can hold, not including the trailing '0'
		};
		...
		struct SSOLayout
		{
			...
			{
				tracked<char, 'm'> mnRemainingSize;
			};
		...
		public:
			...
			{
				dst.raw = src.raw;
				tracking::track_raw(dst.raw, sizeof dst.raw);
			}
			inline void Move(Layout& dst, Layout& src) EA_NOEXCEPT
			{
				eastl::swap(dst.raw, src.raw);
				tracking::track_raw(&dst.raw, sizeof dst.raw);
				tracking::track_raw(&src.raw, sizeof src.raw);
			}
			inline void Swap(Layout& a, Layout& b) EA_NOEXCEPT
			{
				eastl::swap(a.raw, b.raw);
				tracking::track_raw(&a.raw, sizeof a.raw);
				tracking::track_raw(&b.raw, sizeof b.raw);
			}

			inline void ResetToSSO() EA_NOEXCEPT { tracking::memset(&raw, 0, sizeof(RawLayout)); }
		};
		...
		: mPair(allocator)
	{
		...
		*internalLayout().EndPtr() = 0;
		tracking::track_raw(internalLayout().EndPtr(), sizeof *internalLayout().EndPtr());
	}
	...
	inline void basic_string<T, Allocator>::set_capacity(size_type n)
	{
		...
		if((n < capacity() && internalLayout().IsHeap()) || (n > capacity()))
		{
			...
			if(EASTL_LIKELY(n))
			{
				...
				*pNewEnd = 0;
				tracking::track_raw(pNewEnd, sizeof *pNewEnd);
	...
	inline void basic_string<T, Allocator>::clear() EA_NOEXCEPT
	{
		...
		*internalLayout().BeginPtr() = value_type(0);
		tracking::track_raw(internalLayout().BeginPtr(), sizeof *internalLayout().BeginPtr());
	}
	...
	inline void basic_string<T, Allocator>::pop_back()
	{
		...
		tracking::track_raw(&internalLayout().EndPtr()[-1], sizeof internalLayout().EndPtr()[-1]);
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::assign(const value_type* pBegin, const value_type* pEnd)
	{
		...
		if(n <= internalLayout().GetSize())
		{
			tracking::memmove(internalLayout().BeginPtr(), pBegin, (size_t)n * sizeof(value_type));
			erase(internalLayout().BeginPtr() + n, internalLayout().EndPtr());
		}
		else
		{
			tracking::memmove(internalLayout().BeginPtr(), pBegin, (size_t)(internalLayout().GetSize()) * sizeof(value_type));
	...
	inline basic_string<T, Allocator>& basic_string<T, Allocator>::assign(this_type&& x)
	{
		if(get_allocator() == x.get_allocator())
		{
			...
			tracking::track_raw(&internalLayout(), sizeof internalLayout());
			tracking::track_raw(&x.internalLayout(), sizeof x.internalLayout());
		}
		else
			assign(x.internalLayout().BeginPtr(), x.internalLayout().EndPtr());
		...
		if(n) // If there is anything to insert...
		{
			if(internalLayout().GetRemainingCapacity() >= n) // If we have enough capacity...
			{
				...
				if(nElementsAfter >= n) // If there's enough space for the new chars between the insert position and the end...
				{
					...
					tracking::memmove(const_cast<value_type*>(p) + n, p, (size_t)((nElementsAfter - n) + 1) * sizeof(value_type));
		...
		if(n)
		{
			...
			if(bCapacityIsSufficient && !bSourceIsFromSelf)
			{
				...
				if(nElementsAfter >= n) // If there are enough characters between insert pos and end
				{
					...
					internalLayout().SetSize(nSavedSize + n);
					tracking::memmove(const_cast<value_type*>(p) + n, p, (size_t)((nElementsAfter - n) + 1) * sizeof(value_type));
					tracking::memmove(const_cast<value_type*>(p), pBegin, (size_t)(n) * sizeof(value_type));
				}
				else
				{
					pointer pOldEnd = internalLayout().EndPtr();
	...
	basic_string<T, Allocator>::erase(const_iterator p)
	{
		...
		tracking::memmove(const_cast<value_type*>(p), p + 1, (size_t)(internalLayout().EndPtr() - p) * sizeof(value_type));
		internalLayout().SetSize(internalLayout().GetSize() - 1);
	...
	basic_string<T, Allocator>::erase(const_iterator pBegin, const_iterator pEnd)
	{
		...
		if(pBegin != pEnd)
		{
			tracking::memmove(const_cast<value_type*>(pBegin), pEnd, (size_t)((internalLayout().EndPtr() - pEnd) + 1) * sizeof(value_type));
			const size_type n = (size_type)(pEnd - pBegin);
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::replace(const_iterator pBegin1, const_iterator pEnd1, const value_type* pBegin2, const value_type* pEnd2)
	{
		...
		if(nLength1 >= nLength2) // If we have a non-expanding operation...
		{
			if((pBegin2 > pEnd1) || (pEnd2 <= pBegin1))  // If we have a non-overlapping operation...
				tracking::memcpy(const_cast<value_type*>(pBegin1), pBegin2, (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
			else
				tracking::memmove(const_cast<value_type*>(pBegin1), pBegin2, (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
		...
		else // Else we are expanding.
		{
			if((pBegin2 > pEnd1) || (pEnd2 <= pBegin1)) // If we have a non-overlapping operation...
			{
				...
				if((pEnd2 <= pBegin1) || (pBegin2 > pEnd1))
					tracking::memcpy(const_cast<value_type*>(pBegin1), pBegin2, (size_t)(pMid2 - pBegin2) * sizeof(value_type));
				else
					tracking::memmove(const_cast<value_type*>(pBegin1), pBegin2, (size_t)(pMid2 - pBegin2) * sizeof(value_type));
	...
	void basic_string<T, Allocator>::swap(this_type& x)
	{
		...
		else // else swap the contents.
		{
			...
			*this = x;                   // itself call this member swap function.
			x     = temp;
			tracking::track_raw(this, sizeof *this);
			tracking::track_raw(&x, sizeof x);
		}
	}
	...
	inline void basic_string<T, Allocator>::make_lower()
	{
		...
		tracking::track_raw(r, (internalLayout().EndPtr() - r) * sizeof *r);
	}
	...
	basic_string<T, Allocator>::InsertInternal(const_iterator p, value_type c)
	{
		...
		{
			const size_type nSavedSize = internalLayout().GetSize();
			tracking::memmove(const_cast<value_type*>(p) + 1, p, (size_t)(internalLayout().EndPtr() - p) * sizeof(value_type));
			*(internalLayout().EndPtr() + 1) = 0;
			tracking::track_raw(internalLayout().EndPtr() + 1, sizeof *(internalLayout().EndPtr() + 1));
			*pNewPosition = c;
			tracking::track_raw(pNewPosition, sizeof *pNewPosition);
	...
	void basic_string<T, Allocator>::SizeInitialize(size_type n, value_type c)
	{
		...
		internalLayout().SetSize(n);
		*internalLayout().EndPtr() = 0;
		tracking::track_raw(internalLayout().EndPtr(), *internalLayout().EndPtr());
	}
