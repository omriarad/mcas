...
namespace eastl
{
	...
	public:
		tracked<this_type*, true ? 'T' : 'r'> mpNodeRight;  // Declared first because it is used most often.
		tracked<this_type*, true ? 'T' : 'l'> mpNodeLeft;
		tracked<this_type*, true ? 'T' : 'p'> mpNodeParent;
		tracked<enum RBTreeColor, 'C'> mColor; // We only need one bit here, would be nice if we could stuff that bit somewhere else.
	};
	...
	{
		auto* pNode = static_cast<node_type*>(allocate_memory(mAllocator, sizeof(node_type), EASTL_ALIGN_OF(value_type), 0));
