...
#include <EASTL/internal/tracked.h> // tracking::memset etc.
...
namespace eastl
{
	...
	{
		tracking::memmove(pDestination, pSource, (size_t)(pSourceEnd - pSource) * sizeof(T));
	...
	inline char8_t* CharStringUninitializedFillN(char8_t* const pDestination, size_t n, const char8_t c)
	{
		if(n) // Some compilers (e.g. GCC 4.3+) generate a warning (which can't be disabled) if you call memset with a size of 0.
			tracking::memset(pDestination, (uint8_t)c, (size_t)n);
		return pDestination + n;
	}

	template<typename T>
	inline T* CharStringUninitializedFillN(T* const pDestination, size_t n, const T c)
	{
		T * pDest           = pDestination;
		const T* const pEnd = pDestination + n;
		while(pDest < pEnd)
			*pDest++ = c;
		tracking::track_raw(pDestination, (pDest - pDestination) * sizeof *pDestination);
		return pDestination + n;
	}


	inline char8_t* CharTypeAssignN(char8_t* const pDestination, size_t n, char8_t c)
	{
		if(n) // Some compilers (e.g. GCC 4.3+) generate a warning (which can't be disabled) if you call memset with a size of 0.
			return (char8_t*)tracking::memset(pDestination, c, (size_t)n);
		return pDestination;
	}

	template<typename T>
	inline T* CharTypeAssignN(T* const pDestination, size_t n, T c)
	{
		T* pDest            = pDestination;
		const T* const pEnd = pDestination + n;
		while(pDest < pEnd)
