...
namespace eastl
{
	...
	EA_DISABLE_VC_WARNING(4625 4626) // "copy constructor / assignment operator could not be generated because a base class copy constructor is inaccessible or deleted"
		...
		struct hash_node<Value, true>
		{
			...
			Value        mValue;
			tracked<hash_node*, 'H'>   mpNext;
			tracked<eastl_size_t, 'c'> mnHashCode;      // See config.h for the definition of eastl_size_t, which defaults to size_t.
		} EASTL_MAY_ALIAS;
	...
	protected:
		tracked<node_type**, 'H'>    mpBucketArray;
		tracked<size_type, 'S'>      mnBucketCount;
		tracked<size_type, 'S'>      mnElementCount;
					...
					for(size_type i = 0; i < x.mnBucketCount; ++i)
					{
						...
						while(pNodeSource)
						{
							*ppNodeDest = DoAllocateNode(pNodeSource->mValue);
							...
							ppNodeDest = &(*ppNodeDest)->mpNext.value();
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::erase(const key_type& k)
	{
		...
		while(*pBucketArray && !compare(k, c, *pBucketArray))
			pBucketArray = &((*pBucketArray)->mpNext).value();
	...
	inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::reset_lose_memory() EA_NOEXCEPT
	{
		...
		mnBucketCount  = 1;
			...
			tracking::memcpy(&mpBucketArray, &p, sizeof(mpBucketArray)); // Other compilers implement strict aliasing and casting is thus unsafe.
		#endif
