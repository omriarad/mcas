...
static constexpr auto count_inner = 100U;
...
void write_read_sequential_client(component::IFabric & fabric_, const unsigned iter0_, const uint16_t control_port_, const char *const remote_host_, bool force_error_) // const std::string &fabric_spec_, const char *const remote_host, uint16_t control_port_, bool force_error_
{
	...
	std::array<unsigned, 64> ct_w{};
	std::array<unsigned, 64> ct_r{};
	std::cerr << std::dec;
...
#else
			client.write_uninitialized(same_size_as_fabtest, force_error_);
#endif
			auto t = double_seconds(t_write.elapsed());
			++ct_w.at(unsigned(std::log2(t*1e6))); /* element 0 is [1..2) microsecond */
			longish_write += ( 1.0 <= t );
			long_write += ( 2.0 <= t );
	...
	std::cerr << "Data rate " << std::dec << data_size << " bytes in " << double_seconds(t_duration) << " seconds " << double(data_size) / 1e9 / double_seconds(t_duration) << " GB/sec" << " lw " << longish_write << "/" << long_write <<  " lr " << longish_read << "/" << long_read << "\n";
	std::cerr << "wr "; for ( auto i : ct_w ) { std::cerr << i << " "; }; std::cerr << "\n";
	std::cerr << "rd "; for ( auto i : ct_r ) { std::cerr << i << " "; }; std::cerr << "\n";
