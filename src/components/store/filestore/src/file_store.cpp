...
struct Pool_handle {
  ...
  status_t resize_value(const IKVStore::pool_t pool,
                        const std::string &    key,
                          ...
                          size_t &           out_value_len)
{
  ...
  if (rs != out_value_len) throw General_exception("file read failed");
                                 ...
                                 size_t &           out_value_len)
{
  ...
  if (out_value_len < buffer.st_size) return E_INSUFFICIENT_BUFFER;
...
status_t Pool_handle::resize_value(const IKVStore::pool_t pool,
                                   const std::string &    key,
                                   const size_t           new_size,
                                   const size_t           alignment)
{
  if (new_size == 0) throw API_exception("bad parameter");
  /*
  ...
  */
  ...
  auto rc = ftruncate(fd, new_size);
  if (rc != 0)
    throw General_exception("file_store::lock ::ftruncate failed %s",
                            strerror(errno));
                                  ...
                                  size_t &              out_value_len)
{
  ...
  if (!fs::exists(full_path)) {
    ...
    int fd = open(full_path.c_str(), O_WRONLY | O_APPEND | O_CREAT, 0644);
    if (fd == -1)
      throw General_exception("file_store::lock ::open failed %s",
                              strerror(errno));
  ...
  if (!created) { /* reload from file */
    ...
    if (rs != out_value_len)
      throw General_exception(
          "file read failed (rs=%lu) != (out_value_len=%lu)", rs,
          out_value_len);
  }
...
{
  Simulated_locked_item *item =
      reinterpret_cast<Simulated_locked_item *>(key_handle);
  assert(item);
  ...
  if (ws != item->p_len)
    throw General_exception("file write failed (%s) in Pool_handle::unlock",
                            strerror(errno));

  /* turn on to avoid the effect of file cache*/
  if (!use_cache) fsync(item->fd);
...
{
  std::string full_path = path.string() + "/" + key;
  if (!fs::exists(full_path)) return IKVStore::E_KEY_NOT_FOUND;
  ...
  switch (attr) {
    ...
    case IKVStore::Attribute::COUNT: {
      ...
      if (is_directory(dir)) {
        ...
        throw Logic_exception("iterating dir");
      return S_OK;
    }
    default:
      return E_BAD_PARAM;
                          ...
                          Component::IKVStore::key_t &     out_key)
{
  ...
  if (_pool_sessions.count(handle) != 1) return E_INVAL;

  out_key = handle->lock(key, type, out_value, out_value_len);
  return S_OK;
}

status_t File_store::unlock(const pool_t pool, IKVStore::key_t key_handle)
{
  auto handle = reinterpret_cast<Pool_handle *>(pool);
  if (_pool_sessions.count(handle) != 1) return E_POOL_NOT_FOUND;
                                         ...
                                         uint64_t           args)
{
  ...
  if (fs::exists(p)) {
    ...
    throw General_exception("filestore unable to create dir (%s)",
                            p.string().c_str());
  }

  if (option_DEBUG) PLOG("created pool OK: %s", p.string().c_str());
                                       ...
                                       unsigned int       flags)
{
  ...
  fs::path p = _root_path + name;
  if (!fs::exists(p)) return POOL_ERROR;

  if (option_DEBUG) PLOG("opened pool OK: %s", p.string().c_str());
...
status_t File_store::delete_pool(const std::string &name)
{
  ...
  if (!fs::exists(fullpath)) return E_POOL_NOT_FOUND;

  lock_guard g(_pool_sessions_lock);
  for (auto &s : _pool_sessions) {
    if (s->path == fullpath) return E_ALREADY_OPEN;
  }
...
{
  auto handle = reinterpret_cast<Pool_handle *>(pool);
  if (_pool_sessions.count(handle) != 1) throw API_exception("bad pool handle");
                                ...
                                Component::IKVStore::memory_handle_t handle)
{
  ...
  if (_pool_sessions.count(pool_handle) != 1) return E_POOL_NOT_FOUND;

  return pool_handle->get_direct(key, out_value, out_value_len);
}

status_t File_store::resize_value(const pool_t       pool,
                                  const std::string &key,
                                  const size_t       new_size,
                                  const size_t       alignment)
{
  ...
  return pool_handle->resize_value(pool, key, new_size, alignment);
}

status_t File_store::erase(const pool_t pool, const std::string &key)
{
  auto handle = reinterpret_cast<Pool_handle *>(pool);
