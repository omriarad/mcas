...
MCAS_client::MCAS_client(unsigned           debug_level,
...
int MCAS_client::thread_safety() const
{
...
MCAS_client::register_direct_memory(void*  vaddr,
                                    size_t len)
{
  madvise(vaddr, len, MADV_DONTFORK);
  // if(madvise(vaddr, len, MADV_DONTFORK) != 0) {
  //   PWRN("MCAS_client::register_direct_memory:: madvise 'don't fork' failed unexpectedly (%p %lu) %s",
...
size_t MCAS_client::count(const IKVStore::pool_t pool)
{
...
status_t MCAS_client::get_statistics(Shard_stats& out_stats)
{
...
{
  return _connection->find(pool, key_expression, offset,
                           out_matched_offset, out_matched_key);
}
...
{
  return _connection->invoke_ado(pool, key, request, request_len,
