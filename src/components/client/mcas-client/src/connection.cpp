                                            ...
                                            const std::string& json)
{
  ...
  const auto response_msg =
    response_ptr<const mcas::Protocol::Message_IO_response>(iobr->base());

  if (option_DEBUG)
    PLOG("got response from CONFIGURE operation: status=%d request_id=%lu",
         response_msg->get_status(), response_msg->request_id);
                                 ...
                                 uint32_t flags)
{
  ...
  if (option_DEBUG)
    PINF("put: %.*s (key_len=%lu) (value_len=%lu)", (int) key_len, (const char*) key,
         key_len, value_len);
                                                  ...
                                                  uint32_t                             flags)
{
...

  {
    ...
    if(option_DEBUG)
      PMAJOR("got response (status=%u) from put direct header",response_msg->get_status());
                                        ...
                                        uint32_t                             flags)
{
  ...
  try {
    ...
    if (option_DEBUG)
      PLOG("got response from PUT_DIRECT operation: status=%d", msg->get_status());
                                 ...
                                 std::string&       value)
{
  ...
  try {
    ...
    if (option_DEBUG)
      PLOG("got response from GET operation: status=%d (%s)", msg->get_status(),
           response_msg->data);
...
size_t Connection_handler::count(const pool_t pool)
{
  ...
  try {
    ...
    const auto response_msg =
      response_ptr<const mcas::Protocol::Message_INFO_response>(iobr->base());
...
status_t Connection_handler::get_statistics(Component::IMCAS::Shard_stats& out_stats)
{
  API_LOCK();
...
status_t Connection_handler::invoke_ado(const Component::IKVStore::pool_t pool,
                                        const std::string& key,
                                        const void * request,
                                        size_t request_len,
                                        ...
                                        const size_t value_size)
{
  ...
  try {
    ...
    const auto response_msg =
      response_ptr<const mcas::Protocol::Message_ado_response>(iobr->base());
...
int Connection_handler::tick()
{
  ...
  case SHUTDOWN: {
    ...
    auto msg = new (iob->base())
      mcas::Protocol::Message_close_session((uint64_t) this);
