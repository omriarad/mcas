 ...
 */
class Crypto_session_base : public ICrypto::Opaque_crypto_session {
 public:
  /**
...
/**
 * @brief      Server-side for cert+key session
 */
class Cert_server_session : public Crypto_session_base {
 public:
  // Cert_server_session() : _state(nullptr) {} // false constructor

  Cert_server_session(unsigned debug_level, const std::shared_ptr<Crypto_server_state> state, int port)
      : Crypto_session_base(true, S_OK), _debug_level(debug_level), _state(state)
  {
    ...
    /* examine peer (client) x509 certificate */
    {
      ...
      if (debug_level > 0) PLOG("Client Cert DN:%s", dn);
    }
   ...
   if (cert_list_size > 0 && gnutls_certificate_type_get(session) == GNUTLS_CRT_X509)
     {
       ...
       int rc = gnutls_x509_crt_get_dn (cert, dn, &dn_size);
       if( rc == GNUTLS_E_REQUESTED_DATA_NOT_AVAILABLE)
        PINF("- DN: not available");
       else 
        PINF("- DN: (%d) %s", rc, dn);
  
       rc = gnutls_x509_crt_get_issuer_dn (cert, dn, &dn_size);
       if( rc == GNUTLS_E_REQUESTED_DATA_NOT_AVAILABLE)
