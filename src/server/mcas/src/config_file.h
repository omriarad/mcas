...
class Config_exception : public Exception {
 public:
  ...
  __attribute__((__format__(__printf__, 2, 0)))
    Config_exception(const char *fmt, ...) : Exception(""), _err_code(E_FAIL)  {
    va_list args;
    va_start(args, fmt);
    char msg[255] = {0};
...
public:
  Config_file(const std::string &filename) : _doc() {
    ...
    if (!_shards.IsArray())
      throw Config_exception("bad JSON: shard should be array");

    for (auto &m : _shards.GetArray()) {
      if (!m["core"].IsInt())
        throw Config_exception("bad JSON: shards::core member not integer");
      if (!m["port"].IsInt())
        throw Config_exception("bad JSON: shards::port member not integer");
    ...
    if (_doc.HasMember("resources")) { /* optional */
      ...
      if (!_resources.IsObject())
        throw Config_exception("bad JSON: resources should be {..}");
      if (!_resources["ado_cores"].IsString())
        throw Config_exception("bad JSON: resources::core members not string");
    ...
    if (option_DEBUG) {
      PLOG("net_providers: %s%s",
           (nit == _doc.MemberEnd() ? "(default) " : " "),
           get_net_providers().c_str());
    }
  }
  ...
  auto get_shard(rapidjson::SizeType i) const {
    if (i > shard_count())
      throw Config_exception("get_shard out of bounds");
  ...
  float get_shard_ado_core_nu(rapidjson::SizeType i) const {
    ...
    assert(_shards[i].IsObject());
    auto shard = _shards[i].GetObject();
    if (!shard.HasMember("ado_core_number") && shard.HasMember("ado_core"))
      return -1;
    if (!shard.HasMember("ado_core_number"))
      return 1;
  ...
  std::string get_shard(std::string name, rapidjson::SizeType i) const {
    if (i > shard_count())
      throw Config_exception("get_shard out of bounds");
    if (name.empty())
      throw Config_exception("get_shard invalid name");
  ...
  auto get_shard_object(std::string name, rapidjson::SizeType i) const {
    if (i > shard_count())
      throw Config_exception("get_shard_object out of bounds");
    if (name.empty())
      throw Config_exception("get_shard_object invalid name");
    auto shard = get_shard(i);
    if (!shard.HasMember(name.c_str()))
      throw Config_exception("get_shard_object: object (%s) does not exist",
                              name.c_str());
    return shard[name.c_str()].GetObject();
  }

  std::vector<std::pair<std::string, std::string>>
  get_shard_dax_config(rapidjson::SizeType i) const {
    if (i > shard_count())
      throw Config_exception("get_shard_dax_config out of bounds");
  ...
  std::string get_cert_path() const {
    return (!_doc.HasMember("security")) ?
      std::string() : std::string(_security["cert"].GetString());
  }

  unsigned int debug_level() const {
    if (_doc["debug_level"].IsNull())
      return 0;
