...
public:
  Config_file(const std::string &filename) : _doc() {
    ...
    if (_doc.HasMember("resources")) {
      try {
        ...
        throw General_exception("bad JSON in configuration file (resources))");
      }

      PLOG("resource type:%s", k_typenames[_resources.GetType()]);
      if (!_resources.IsObject())
        throw General_exception("bad JSON: resources should be {..}");

      if (!_resources["ado_cores"].IsString())
        throw General_exception("bad JSON: resources::core members not string");
      if (_resources.HasMember("ado_manager_core"))
        if (!_resources["ado_manager_core"].IsInt())
          throw General_exception(
              "bad JSON: optional resources::ado manager core member not int");
    }
  ...
  auto get_shard(rapidjson::SizeType i) const {
    if (i > shard_count())
      throw General_exception("get_shard out of bounds");
  ...
  std::string get_shard(std::string name, rapidjson::SizeType i) const {
    if (i > shard_count())
      throw General_exception("get_shard out of bounds");
    if (name.empty())
      throw General_exception("get_shard invalid name");
  ...
  auto get_shard_object(std::string name, rapidjson::SizeType i) const {
    ...
    if (name.empty())
      throw General_exception("get_shard_object invalid name");
    auto shard = get_shard(i);
    if (!shard.HasMember(name.c_str()))
