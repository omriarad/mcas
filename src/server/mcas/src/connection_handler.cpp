...
int Connection_handler::tick()
{
  ...
  case WAIT_RECV_VALUE: {
    ...
    if (check_for_posted_value_complete()) {
      ...
      if (option_DEBUG > 2)
        PMAJOR("Shard State: %lu %p WAIT_RECV_VALUE_COMPLETE", _tick_count, this);
  ...
  case POST_HANDSHAKE: {
    ...
    for(size_t i=0;i<NUM_SHARD_BUFFERS/2;i++)
      post_recv_buffer(allocate());
  ...
  case WAIT_HANDSHAKE: {
    if (check_for_posted_recv_complete()) {
      if (option_DEBUG > 2)
        PMAJOR("Shard State: %lu %p WAIT_HANDSHAKE complete", _tick_count, this);
