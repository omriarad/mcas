...
class Fabric_transport {
 ...
 public:
                   ...
                   unsigned          port)
  {
    ...
    if (option_DEBUG)
      PLOG("fabric_transport: (provider=%s, device=%s, port=%u)",
           provider.c_str(), device.c_str(), port);
 ...
 private:
            ...
            unsigned           port)
  {
    ...
    /* FABRIC */
    auto i_fabric_factory = static_cast<IFabric_factory*>(
        load_component("libcomponent-fabric.so", net_fabric_factory));

    if (!i_fabric_factory)
      throw General_exception("unable to load Fabric Comanche component");
    ...
    _server_factory = _fabric->open_server_factory(server_factory_spec,
                                                   boost::numeric_cast<uint16_t>(port));
