 ...
 protected:
  ...
  bool check_for_posted_send_complete()
  {
    ...
    if (_posted_send_buffer) {
      if(option_DEBUG > 2)
        PLOG("Fabric_connection_base::freeing buffer (%p)", _posted_send_buffer);
  ...
  void post_recv_buffer(buffer_t *buffer)
  {
    ...
    _transport->post_recv(buffer->iov,
                          buffer->iov + 1,
                          &buffer->desc, buffer);
  ...
  void post_send_buffer(buffer_t *buffer, buffer_t *val_buffer = nullptr)
  {
    ...
    if (!val_buffer) {
      ...
      if (iov->iov_len <= _transport->max_inject_size()) {
        ...
        if(option_DEBUG > 2)
          PLOG("Fabric_connection_base: posting send with inject (%p)", iov->iov_base);
        ...
        if(option_DEBUG > 2)
          PLOG("Fabric_connection_base: posting send (%p, %p)", buffer, iov->iov_base);
  ...
  {
    if( _posted_recv_buffer_count > 0 ||
