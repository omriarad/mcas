...
public:
  Shard_security_state(const std::string& certs_path) {
    ...
    try {
      ...
      _cert = PEM_read_X509(fp,
                            nullptr,
                            nullptr /* passwd callback */,
                            nullptr);
      if(_cert) {
        /* check certificate */
        ...
        if(_debug_level > 0)
          PLOG("X509 certificate OK.");
      }

      fclose(fp);

      std::ifstream t(certs_path);
      _cert_base64.assign(std::istreambuf_iterator<char>(t),
...
{
  if(_debug_level > 1)
      PMAJOR("Shard_security: auth=%s cert path (%s)", _auth_enabled ? "y":"n", certs_path.c_str());
