...
void ADO_manager::init() {
  ...
  if (core == -1) {
    for (auto i : _ado_cpu_pool)
      _manager_cpu_pool.insert(i);
    for (unsigned i = 0; i < shard_count(); i++) {
      set<unsigned> cores;
      string_to_set(get_shard_ado_core(i), cores);
      for (auto j : cores)
        _manager_cpu_pool.erase(j);
    }
  }
...
void ADO_manager::schedule(unsigned int shard, string cpus, float cpu_num,
