...
# launch client
[ 0 -lt $DEBUG ] && echo ./dist/testing/ado-test --provider sockets --src_addr "$NODE_IP" --server $NODE_IP --port 11911
./dist/testing/ado-test --provider sockets --src_addr "$NODE_IP" --server $NODE_IP --port 11911 &> $CLIENT_LOG &
CLIENT_PID=$!
