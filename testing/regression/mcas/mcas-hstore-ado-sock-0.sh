...
# launch MCAS server
DAX_RESET=1 ./dist/bin/mcas --conf ./dist/testing/testing-ado0-sock.conf --debug 0 &> test$TESTID-server.log &
SERVER_PID=$!
...
#launch client
./dist/bin/ado-test --port 11921 --server $NODE_IP &> $CLIENT_LOG &
CLIENT_PID=$!
