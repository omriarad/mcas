...
# launch MCAS server
echo DAX_RESET=1 ./dist/bin/mcas --config \'"$("./dist/testing/$SERVER_CONFIG.py" "$NODE_IP")"\' --forced-exit --debug $DEBUG
...
echo ./dist/bin/kvstore-perf --cores "$(clamp_cpu 14)" --src_addr $NODE_IP --server $NODE_IP --test put --component mcas --elements $ELEMENT_COUNT --size $STORE_SIZE --skip_json_reporting --key_length 8 --value_length $VALUE_LENGTH --debug_level $DEBUG
