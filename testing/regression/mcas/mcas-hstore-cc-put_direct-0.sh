...
# launch client
RECOMMENDED_ELEMENT_COUNT=$(scale_by_transport 3000)
ELEMENT_COUNT=${ELEMENT_COUNT:-$RECOMMENDED_ELEMENT_COUNT}
RECOMMENDED_STORE_SIZE=$((ELEMENT_COUNT*(8+VALUE_LENGTH)*13/10)) # too small
RECOMMENDED_STORE_SIZE=$((ELEMENT_COUNT*(8+VALUE_LENGTH)*20/10)) # sufficient
STORE_SIZE=${STORE_SIZE:-$RECOMMENDED_STORE_SIZE}
CLIENT_LOG="test$TESTID-client.log"
[ 0 -lt $DEBUG ] && echo ./dist/bin/kvstore-perf --cores "$(clamp_cpu 14)" --src_addr $NODE_IP --server $NODE_IP --test put_direct --component mcas --elements $ELEMENT_COUNT --size $STORE_SIZE --skip_json_reporting --key_length 8 --value_length $VALUE_LENGTH --debug_level $DEBUG
./dist/bin/kvstore-perf --cores "$(clamp_cpu 14)" --src_addr $NODE_IP --server $NODE_IP --test put_direct --component mcas --elements $ELEMENT_COUNT --size $STORE_SIZE --skip_json_reporting --key_length 8 --value_length $VALUE_LENGTH --debug_level $DEBUG &> $CLIENT_LOG &
CLIENT_PID=$!
