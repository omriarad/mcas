...
Author      : Andrew Wood + Moshik Hershcovitch
Author_email: dewood@bu.edu + moshikh@il.ibm.com
...
def train_one_epoch(m: pt.nn.Module,
                    ...
                    cuda: int) -> None:
    i = 1
    for batch_idx, (s.X, s.Y_gt) in tqdm(enumerate(loader),
                                     desc="training epoch %s" % epoch,
                                     total=len(loader)):
        optim.zero_grad()
        if requires_flat_examples(m):
            print("requires_flat_examples")
            s.X = s.X.clone().view(s.X.size(0), -1)
#        if cuda > -1:
#            s.X2 = s.X.to(cuda)
        print("problem here")
        s.Y_hat: pt.Tensor = m.forward(s.X)
        loss: pt.Tensor = F.nll_loss(s.Y_hat.cpu(), s.Y_gt.cpu())
        s.erase('Y_hat')
        exit(0)
        loss.backward()
        optim.step()
...
def main() -> None:
    ...
    parser.add_argument("-back", "--backend", type=str,
                        default="hstore", choices=["hstore", "mapstore"],
                        help="The backend of the shelf - hstore/ mapstore")
...
#        args.posterior_sample_size = len(train_loader) * args.epochs
    ...
    start_time = time.time()
    print ("mnist_pymm_tensor, ", "size_mb=" + str(args.size_mb) + ", pmem_path=" + str(args.shelf_file) +", backend=" + str(args.backend) + ", force_new=True")

    shelf = pymm.shelf("mnist_pymm_tensor",
